 ...
 */
export default function RotationTimeline<TState>({
  SpellBarComponent,
  GaugeComponent,
  simulate,
  job
}: RotationTimelineBaseProps<TState>) {
  ...
  useEffect(() => {
    ...
    async function loadRotation() {
      ...
      const data = await getRotation(groupId, fightId, job)
  ...
  useEffect(() => {
    ...
    const save = async () => {
      ...
      await saveRotation(groupId, fightId, job, {
        actions,
        downtimes,
        spellSpeed,
        timelineStart
  ...
  return (
    <PageDiv>
      <div>
        <GaugeComponent playerState={playerState} />
      </div>
      <TimelineDiv>
        <TimelineInfo>
